...
<%}else{ 
	for(int i=0;i < vlist.size();i++){
		...
		//방명록 글쓴이
		JoinBean write = mgr.getJoin(bean.getId());
		%>
			...
			<table bgcolor="#F5F5F5">
				<tr>
					...
					<td width="225">
						...
						<a href="mailto:"<%=write.getEmail()%> >
							<%=write.getName()%>
						</a>
					</td>					
					<td width="150" align="center">
						<a href="http://<%=write.getHp()%>">
				...
				<tr>
					<td>IP : <%=bean.getIp()%></td>					
					<td><%=bean.getRegdate()+" "+bean.getRetime()%></td>					
					<td>
						<!-- 수정, 삭제 : 로그인 id와 write의 id가 동일할 경우 -->
						<!-- 삭제 : 관리자-->
						<!-- 비밀글 : secret 1 일때 -->
						<%
						boolean chk = login.getId().equals(write.getId());
